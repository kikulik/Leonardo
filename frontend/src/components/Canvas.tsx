...
const BOX_W = 160;
// BODY dimensions only (header is a separate element)
const BOX_H = 80;

const HEADER_H = 36; // purely visual, separate above body
...
export function Canvas({
  ...
  // Track the visible canvas size so we can expand the SVG viewBox.
  ...
  function handlePortClick(d: Device, p: Port) {
    if (!pending) { setPending({ from: { deviceId: d.id, portName: p.name } }); setCursorWorld(null); return; }
    if (pending.from.deviceId === d.id && pending.from.portName === p.name) { setPending(null); setCursorWorld(null); return; }
    ...
    if (aPort.direction === "OUT" && bPort.direction === "IN") { fromEnd = { deviceId: aDev.id, portName: aPort.name }; toEnd = { deviceId: bDev.id, portName: bPort.name }; }
    else if (aPort.direction === "IN" && bPort.direction === "OUT") { fromEnd = { deviceId: bDev.id, portName: bPort.name }; toEnd = { deviceId: aDev.id, portName: aPort.name }; }
    else { setPending({ from: { deviceId: d.id, portName: p.name } }); return; }

    onChange(addConnection(graph, fromEnd, toEnd));
    setPending(null); setCursorWorld(null);
  }

  // BODY ports SVG
  function DevicePortsSVG({ d }: { d: Device }) {
    const w = d.w ?? BOX_W, h = d.h ?? BOX_H;
    const INs = (d.ports ?? []).filter((p) => p.direction === "IN");
    const OUTs = (d.ports ?? []).filter((p) => p.direction === "OUT");
    ...
    return (
      <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} className="absolute inset-0">
        {INs.map((p, idx) => {
          ...
          return (
            <g key={p.id} className="cursor-crosshair" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); handlePortClick(d, p); }}>
  ...
  // Big margin around the canvas so negative coords aren’t clipped by the SVG viewport.
  const VB_MARGIN = 20000; // generous cushion to cover off-screen content
  ...
  return (
    <div className="relative w-full h-full select-none" onWheel={onWheel}>
      ...
      >
        {/* Connection overlay:
            - Huge viewBox so negative world coords render.
            - overflow: visible ensures curves aren’t clipped by SVG viewport. */}
        <svg
          className="absolute"
        ...
        {/* DEVICES (header separate; unchanged) */}
        {devices.map((d) => {
          const w = d.w ?? BOX_W, h = d.h ?? BOX_H;
